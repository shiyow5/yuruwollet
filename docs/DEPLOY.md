...
**ダッシュボードから手で作ろうとしないこと。作れない。**
Access の self-hosted アプリは、ドメイン欄が**アカウント内のゾーン**に限定される。
このアカウントのゾーンは **`shiyow.dev` だけ**で（`GET /zones` で確認済み）、`pages.dev` は
**Cloudflare の共有ゾーン**なのでアカウントのゾーンとしては存在しない。
つまり `yuruwollet.pages.dev` を**ダッシュボードの self-hosted アプリのドメインに入力できない**。

**API 経由にはこのゾーン制限が無い。** 現行の本番はこの経路で作られていて、実際に動いている
（Google ログインが通り、`/api/session` が Access JWT を検証し、Access 非経由では 403）:
...
**冪等**なので何度実行しても既存アプリを作り直さない。
**既に動いている本番の Access 設定は触らなくてよい。**

> どうしてもダッシュボードで触りたい場合は、**Pages プロジェクト → Settings → Enable Access policy**
> でアプリを生成させてから、そのアプリを編集する（Pages 側から作らせれば `pages.dev` のホストが入る）。
> ただし本リポジトリはこの経路を使っておらず**未検証**。`make setup-prod` を正とする。
