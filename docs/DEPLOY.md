...
`yuruwollet` を `https://yuruwollet.pages.dev` に公開し、**ゆるり・しよを の 2 人だけ**が
開けるようにするまでの手順。上から順に実行する。

> **カスタムドメイン（`yuruwollet.shiyow.dev` など）は使わない。**
...
### B-3. 【重要】カスタムドメインは割り当てない

**`*.pages.dev` をそのまま使う。**（このプロジェクトなら `https://yuruwollet.pages.dev`）

> ## ⚠️ カスタムドメインを足すと、同じゾーンの他のサイトが 403 になる
>
> 2026-07-13 に実際に踏んだ。`yuruwollet.shiyow.dev` を Pages のカスタムドメインとして
> 追加した直後から、**同じゾーンの `shiyow.dev`（ポートフォリオ）と `dm-ai.shiyow.dev` が
> 断続的に 403** になった。
>
> **症状**（この形なら同じ問題）:
> - **対称的**: どちらを先に開いても、次に開いた方が 403
> - **時間経過で直る**（接続がアイドルで閉じると回復）
> - **シークレットウィンドウでは起きない**
> - 匿名の `curl` では**再現しない**
>
> **原因**:
> 1. Pages のカスタムドメインは **そのホスト専用の証明書**を発行する
>    （ゾーンの Universal SSL `[zone, *.zone]` があっても exact match が優先されて使われない）
> 2. 同一ゾーンのホストは Cloudflare の**同じ IP** を共有する
> 3. ブラウザは同一 IP に対して **HTTP/2 のコネクションを使い回す**
> 4. Cloudflare は「**接続の証明書が、要求されたホストを覆っていない**」場合に **403** を返す
>
> **確定させる 1 行**（SNI と `:authority` を食い違わせる = コネクション再利用と同じ状態）:
> ```bash
> curl --http2 -o /dev/null -w "%{http_code}\n" -H "Host: other.example.com" https://app.example.com/
> #   403 → その証明書は other.example.com を覆っていない（= この問題）
> ```
>
> **証明書側では回避できない。** Pages の個別証明書が Advanced/Universal より優先されるため、
> ワイルドカード証明書を発行しても上書きできない。
> **Cloudflare Access は無関係**（足しても消しても直らない）。
>
> どうしてもカスタムドメインが要るなら、**yuruwollet 専用の別ドメイン/別ゾーン**を使う。
...
### C-3. アプリケーションを作る（`make setup-prod` が API で作る。手作業は不要）

**このリポジトリは API 経由（`make setup-prod`）を正とする。** 現行の本番はこの経路で作られている。

```bash
make setup-prod        # 内部で scripts/setup_access.py が Access を構築し AUD を返す
```

`setup_access.py` が API でやること:

1. **Google IdP を登録**（C-2 の Client ID / Secret を使う。`claims: ["picture"]` 付き）
2. **`yuruwollet` アプリ**（self-hosted / `yuruwollet.pages.dev` / Session `720h` /
   `auto_redirect_to_identity: false`）と、**`two of us` ポリシー**
   （Allow → Include → Emails → ゆるり・しよを の 2 メール）を作る
3. **`yuruwollet public icons` アプリ**を作る（下の note 参照）
4. **AUD タグを stdout に返す** → `setup-prod.sh` がそれをそのまま Pages の `ACCESS_AUD` に入れる
   （**C-4 を手でやる必要は無い**）

> **再実行すると、宣言どおりの設定に上書きされる。**
> アプリを新規に作り直しはしないが、IdP もアプリもポリシーも **PUT で揃えにいく**
> （`setup_access.py` は「手で触られていても直す」設計）。
> **ダッシュボードで手を入れた分は元に戻る**ので、それが困るときは実行しないこと。

#### ダッシュボードから作りたい場合（このリポジトリでは使っていない）

Access の **Domain ドロップダウンはアカウント内のアクティブゾーンに限定**される
（[公式ドキュメント](https://developers.cloudflare.com/cloudflare-one/access-controls/applications/http-apps/self-hosted-public-app/):
*"Domains must belong to an active zone in your Cloudflare account."*）。
このアカウントのゾーンは `shiyow.dev` だけで、**`pages.dev` は Cloudflare が所有するドメイン**
（ユーザーのアカウントのゾーンにはできない）なので、**ドロップダウンには出てこない**。

ただし `*.pages.dev` を Access で守る**公式のダッシュボード手順は Pages 側にある**
（[Pages known issues](https://developers.cloudflare.com/pages/platform/known-issues/)）:

> Pages プロジェクト → **Settings → Enable access policy** → 生成されたアプリの
> **Public hostname → Subdomain** からワイルドカード `*` を消して Save

**この経路は本リポジトリでは使っていない**（`make setup-prod` が API で IdP・2 つのアプリ・
ポリシー・AUD 投入まで一括で片付けるため）。ダッシュボードで作った場合は AUD を自分で控えて
C-4 を手で行うことになる。

> **`setup_access.py` は Access アプリを 2 つ作る。**
> 上記の `yuruwollet`（2 人だけ Allow）に加えて、`yuruwollet public icons` を作り、
> **`apple-touch-icon.png` のパスだけ**を **Bypass**（誰でも認証なしで取得可）にする。
> （`destinations` に複数パスを入れると Cloudflare が 400 `too many destinations` を返すため、
> 1 アプリ 1 パス。iOS の「ホーム画面に追加」が使うのは apple-touch-icon だけなので足りる。）
>
> 理由: iOS の「ホーム画面に追加」はクッキーの載らない経路でアイコンを取りに行くため、
> Access に 302 で弾かれ、頭文字「Y」の代替アイコンになる（#72）。アイコンは公開されて
> 害がないので、そのパスだけ外に出す。パスが具体的なアプリが優先されるので、
> `yuruwollet` 側の守り（2 人だけ）は変わらない。
> **`site.webmanifest` は Bypass しない**（name/description に二人の名前が入るため。
> iOS は manifest を見ないので、これで足りる）。

### C-4. Pages に Access の値を入れて再デプロイ

> **`make setup-prod` を使ったならここは済んでいる。**（C-3 が返した AUD と `ACCESS_TEAM_DOMAIN` を
> Pages の secret に入れ、ビルドしてデプロイするところまで自動でやる。）
> 以下は**後から値を手で変えたとき**の手順。
...
1. **第三者を弾けているか**（最重要）
   - 許可していない Google アカウントで `https://yuruwollet.pages.dev` を開く
